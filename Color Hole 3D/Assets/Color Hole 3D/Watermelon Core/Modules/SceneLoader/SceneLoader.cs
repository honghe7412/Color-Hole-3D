...
namespace Watermelon
{
    ...
    public class SceneLoader : MonoBehaviour
    {
...
#if MODULE_DEBUG_CONSOLE
        [ConsoleInvocableMethod("sceneLoad", "Load scene by id")]
        public void LoadScene(int id)
        {
            LoadScene((Scenes)id);
        }

        [ConsoleInvocableMethod("sceneList", "Show scenes list")]
        public void ShowScenes()
        {
            string[] scenes = System.Enum.GetNames(typeof(Scenes));

            DebugConsole.LogSystem("Game scenes: ");

            for (int i = 0; i < scenes.Length; i++)
            {
                DebugConsole.LogSystem(i + ": " + scenes[i]);
            }
        }
#endif
        ...
        static void CheckInitScene()
        {
            ...
            UnityEditor.EditorApplication.playModeStateChanged += (UnityEditor.PlayModeStateChange playModeState) =>
            {
